...
for num in layer_number:
    ...
    for train, test in kfold.split(train_x, train_y_1D):
        ...
        model.add(Conv1D(25, 3, activation='tanh', input_shape=(12, 1)))
        ...
        model.fit(train_x[train], np_utils.to_categorical(train_y_1D[train], 2),
                  validation_data=(train_x[test], np_utils.to_categorical(train_y_1D[test])),shuffle=True, callbacks=[history], epochs=50, verbose=2)
        # evaluate the model
